...
*/
// Easywall modifications (2026-09-22): preserve empty files/directories, bound EOCD scan and close CRC failures.
var EOCDR = 101010256;
var CDR = 33639248;
var LFR = 67324752;
...
// Incremental ZIP CRC-32 for stored (uncompressed) entries.
var zipCrcTable = [];
for (var crcIndex = 0; crcIndex < 256; crcIndex++) {
    var crcValue = crcIndex;
    for (var crcBit = 0; crcBit < 8; crcBit++) { crcValue = (crcValue >>> 1) ^ ((crcValue & 1) ? 0xedb88320 : 0); }
    zipCrcTable.push(crcValue >>> 0);
}
function crc32(buffer, previous)
{
    var crc = (previous ^ 0xffffffff) >>> 0;
    for (var i = 0; i < buffer.length; i++) { crc = (crc >>> 8) ^ zipCrcTable[(crc ^ buffer[i]) & 255]; }
...
function extractNext(p)
{
    ...
    if (next.endsWith('/') || next.endsWith('\\')) {
        checkFolderPath(dest + (process.platform == 'win32' ? '\\' : '/') + '.directory');
        extractNext(p); return;
    }
    if (p.source._table[next].uncompressedSize === 0) {
        require('fs').closeSync(require('fs').openSync(dest, 'wb'));
        extractNext(p); return;
    }
    ...
    p._output.on('close', function ()
    {
        ...
        {
            this.promise.source.close(); this.promise._rej('CRC Check failed');
            return;
        }
...
function zippedObject(table, descriptor)
{
...
function read(path)
{
    ...
    ret._cdr = function _cdr(err, bytesRead, buffer)
    {
        ...
        {
            if (buffer.length < 46 || ++entryCount > 10000 || buffer.readUInt32LE() != CDR) { require('fs').closeSync(_cdr.self._fd); _cdr.self._rej('Parse Error'); return; }
            var nameLength = buffer.readUInt16LE(28);
            var efLength = buffer.readUInt16LE(30);
            var comLength = buffer.readUInt16LE(32);
            if (46 + nameLength + efLength + comLength > buffer.length || (buffer.readUInt16LE(8) & 1) || [0,8].indexOf(buffer.readUInt16LE(10)) < 0 || buffer.readUInt32LE(20) === 0xffffffff || buffer.readUInt32LE(24) === 0xffffffff) { require('fs').closeSync(_cdr.self._fd); _cdr.self._rej('Unsupported ZIP'); return; }
    ...
    ret._eocdr = function _eocdr(err, bytesRead, buffer)
    {
        ...
        for (i = 22; i <= buffer.length; ++i)
        {
            if ((record = buffer.slice(buffer.length - i)).readUInt32LE() == EOCDR)
            {
                ...
                if(record.readUInt16LE(4)!==0 || record.readUInt16LE(6)!==0 || record.readUInt16LE(10)>10000 || record.readUInt32LE(12)>16777216 || record.readUInt32LE(16)+record.readUInt32LE(12)>_eocdr.self._len) { require('fs').closeSync(_eocdr.self._fd); _eocdr.self._rej('Unsupported ZIP'); return; }
        ...
        require('fs').closeSync(_eocdr.self._fd); _eocdr.self._rej('Invalid ZIP end record');
    };
    ret._cdr.self = ret;
    ret._eocdr.self = ret;
    var tailLength = Math.min(ret._len, 65557);
