...
// Easywall modifications (2026-09-22): preserve empty folders and provide incremental CRC-32.
...
function next(options)
{
    ...
    this._currentFD = fstat.isDirectory() ? null : require('fs').openSync(this._currentFile, require('fs').constants.O_RDONLY);
    this._currentName = this._currentFile.substring(options._baseFolder.length).split('\\').join('/');
    if(fstat.isDirectory()) { this._currentName = this._currentName.split('\\').join('/') + '/'; }
    this._currentFileLength = fstat.isDirectory() ? 0 : fstat.size;
    ...
    if(this._currentFD == null) { this._uncompressedReadSink(null, 0, Buffer.alloc(0)); } else { require('fs').read(this._currentFD, { buffer: this._ubuffer }, this._uncompressedReadSink); }
}
...
function checkFiles(files)
{
    ...
    for(var i in files)
    {
        ...
        else if (s.isDirectory())
        {
            ...
            if(tmp.length === 0) { checked.push(files[i]); }
            for (j in tmp)
            {
...
function write(options)
{
    ...
    ret._uncompressedReadSink = function _uncompressedReadSink(err, bytesRead, buffer)
    {
        ...
        if(self._cancel)
        {
            ...
            try
            {
                if(self._currentFD != null) { require('fs').closeSync(self._currentFD); }
            }
            catch(e)
            {}
